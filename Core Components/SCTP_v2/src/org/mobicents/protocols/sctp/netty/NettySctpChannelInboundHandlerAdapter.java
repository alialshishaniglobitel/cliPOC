...
public class NettySctpChannelInboundHandlerAdapter extends ChannelInboundHandlerAdapter {
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("channelInactive event: association=%s", this.association));
        }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("userEventTriggered event: association=%s \nevent=%s", this.association, evt));
        }
            ...
            switch (not.event()) {
                case COMM_UP:
                    ...
                    if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
                        MyLoggerFactory.getInstance().getAPILogger().info(String.format(
                    ...
                    break;
                case CANT_START:
                    MyLoggerFactory.getInstance().getAPILogger().error(String.format("Can't start for Association=%s", association.getName()));
                    break;
                case COMM_LOST:
                    MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Communication lost for Association=%s", association.getName()));
...
//                    }
                    break;
                case RESTART:
                    MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Restart for Association=%s", association.getName()));
                    try {
                ...
                case SHUTDOWN:
                    if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
                        MyLoggerFactory.getInstance().getAPILogger().info(String.format("Shutdown for Association=%s", association.getName()));
                    }
//                    try {
...
//                    }
                    break;
                default:
                    MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Received unkown Event=%s for Association=%s", not.event(), association.getName()));
                    break;
            }
        }
        ...
        if (evt instanceof PeerAddressChangeNotification) {
            ...
            if (MyLoggerFactory.getInstance().getAPILogger().isWarnEnabled()) {
                MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Peer Address changed to=%s for Association=%s", notification.address(),
                        association.getName()));
            }

        } else if (evt instanceof SendFailedNotification) {
            SendFailedNotification notification = (SendFailedNotification) evt;
            MyLoggerFactory.getInstance().getAPILogger().error(String.format("Association=" + association.getName() + " SendFailedNotification, errorCode="
            ...
            if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
                MyLoggerFactory.getInstance().getAPILogger().info(String.format("Association=%s SHUTDOWN", association.getName()));
            }
...
//            }
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Rx : Ass=%s %s", this.association.getName(), payload));
        }
    ...
    protected void closeChannel() {
        ...
        if (ch != null) {
            try {
                ...
                MyLoggerFactory.getInstance().getAPILogger().error(String.format("Error while trying to close Channel for Associtaion %s",
