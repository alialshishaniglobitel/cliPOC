...
public class NettyAssociationImpl implements Association {
    ...
    public void send(PayloadData payloadData) throws Exception {
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Tx : Ass=%s %s", this.getName(), payloadData));
        }
    ...
    protected void setCongestionLevel(int val) {
        if (this.congLevel != val) {
            MyLoggerFactory.getInstance().getAPILogger().warn("Outgoing congestion control: SCTP: Changing of congestion level for Association=" + this.name + " "
                    + this.congLevel + "->" + val);
        }
    ...
    protected void stop() throws Exception {
        if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().info(String.format("Management requested to stop %s", this.toString()));
        }
        this.started = false;
        for (ManagementEventListener lstr : this.management.getManagementEventListeners()) {
            try {
    ...
    protected void read(PayloadData payload) {
        try {
            ...
            MyLoggerFactory.getInstance().getAPILogger().error(String.format("Error while calling Listener for Association=%s.Payload=%s", this.name, payload), e);
        }
    }
    ...
    protected void scheduleConnect() {
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Scheduling of a channel connection: Association=%s, connectDelay=%d", this,
                    connectDelay));
        }
...
//        final ScheduledExecutorService loop = this.management.getBossGroup().next();
    ...
    protected void connect() {
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Initiating connection started: Association=%s", this));
        }
        ...
        try {
            ...
            MyLoggerFactory.getInstance().getAPILogger().error(String.format("Exception while creating connection for Association=%s", this.getName()), e);
            this.scheduleConnect();
            return;
        }
        ...
        try {
            ...
            if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
                MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Initiating connection scheduled: Association=%s remoteAddress=%s", this,
                        remoteAddress));
            }
        } catch (Exception e) {
            MyLoggerFactory.getInstance().getAPILogger().error(String.format("Exception while finishing connection for Association=%s", this.getName()), e);
        }
    }
