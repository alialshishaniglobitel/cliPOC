...
public class NettyServerImpl implements Server {
    ...
    protected void start() throws Exception {
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().info(String.format("Started Server=%s", this.name));
        }
    }
    ...
    protected void stop() throws Exception {
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().info(String.format("Stoped Server=%s", this.name));
        }
        ...
        if (this.getIpChannel() != null) {
            try {
                ...
                MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Error while stopping the Server=%s", this.name), e);
            }
        }
    }
    ...
    private void initSocket() throws Exception {
        ...
        if (this.ipChannelType == IpChannelType.SCTP) {
            ...
            if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
                MyLoggerFactory.getInstance().getAPILogger().info(String.format("SctpServerChannel bound to=%s ", this.serverChannelSctp.allLocalAddresses()));
            }
        } else {
            this.serverChannelTcp = (NioServerSocketChannel) channelFuture.channel();

            if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
                MyLoggerFactory.getInstance().getAPILogger().info(String.format("ServerSocketChannel bound to=%s ", this.serverChannelTcp.localAddress()));
            }
        }
    }
