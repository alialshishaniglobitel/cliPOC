...
public class NettySctpClientHandler extends NettySctpChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().debug(String.format("channelActive event: association=%s", this.association));
        }
        ...
        if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
            MyLoggerFactory.getInstance().getAPILogger().info(String.format("Association=%s connected to host=%s port=%d", association.getName(), host, port));
        }
