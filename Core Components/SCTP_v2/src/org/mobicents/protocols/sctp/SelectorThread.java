...
public class SelectorThread implements Runnable {
	...
	public void run() {
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info(String.format("SelectorThread for Management=%s started.", this.management.getName()));
		}
		while (this.started) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error("Error while selecting the ready keys", e);
			}
		}
		...
		try {
			...
			MyLoggerFactory.getInstance().getAPILogger().error(String.format("Error while closing Selector for SCTP Management=%s", this.management.getName()));
		}

		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info(String.format("SelectorThread for Management=%s stopped.", this.management.getName()));
		}
	}
	...
	private void doAccept(AbstractSelectableChannel serverSocketChannel, AbstractSelectableChannel socketChannel, Set<SocketAddress> peerAddresses)
			...
			if (srvv.getIpChannel() == serverSocketChannel) { // we have found a server
						...
						if (srv.getName().equals(association.getServerName())) {
							...
							if ((port == association.getPeerPort()) && (inetAddress.getHostAddress().equals(association.getPeerAddress()))) {
								...
								if (!association.isStarted()) {
									MyLoggerFactory.getInstance().getAPILogger().error(String.format("Received connect request for Association=%s but not started yet. Droping the connection! ",
											association.getName()));
									socketChannel.close();
									break;
								}
				...
				if (!provisioned && srv.isAcceptAnonymousConnections() && this.management.getServerListener() != null) {
					...
					if (srvv.getMaxConcurrentConnectionsCount() > 0 && srvv.anonymAssociations.size() >= srvv.getMaxConcurrentConnectionsCount()) {
						MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Incoming anonymous connection is rejected because of too many active connections to Server=%s", srv));
						try {
							socketChannel.close();
						} catch (Exception ee) {
						}
					...
					try {
						...
						MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Exception when invoking ServerListener.onNewRemoteConnection() Ass=%s", anonymAssociation), e);
						try {
							socketChannel.close();
						} catch (Exception ee) {
						}
		...
		if (!provisioned) {
			...
			// channel here
			MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Received connect request from non provisioned %s:%d address. Closing Channel", inetAddress.getHostAddress(), port));
			socketChannel.close();
	...
	private void finishConnectionSctp(SelectionKey key) throws IOException {
		...
		try {
			...
			if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
				MyLoggerFactory.getInstance().getAPILogger().info(String.format("Association=%s connected to=%s", association.getName(), socketChannel.getRemoteAddresses()));
			}

			// Register an interest in writing on this channel
			key.interestOps(SelectionKey.OP_READ);
		} catch (Exception e) {
			MyLoggerFactory.getInstance().getAPILogger().error(String.format("Exception while finishing connection for Association=%s", association.getName()), e);
	...
	private void finishConnectionTcp(SelectionKey key) throws IOException {
		...
		try {
			...
			if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
				MyLoggerFactory.getInstance().getAPILogger().info(String.format("Association=%s connected to=%s", association.getName(), socketChannel.getRemoteAddress()));
			}
