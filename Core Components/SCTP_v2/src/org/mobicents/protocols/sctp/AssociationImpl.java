	...
	protected void start() throws Exception {
		...
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			if (this.type != AssociationType.ANONYMOUS_SERVER) {
				MyLoggerFactory.getInstance().getAPILogger().info(String.format("Started Association=%s", this));
			}
		}
		...
		for (ManagementEventListener lstr : this.management.getManagementEventListeners()) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error("Exception while invoking onAssociationStarted", ee);
			}
		}
	}
	...
	protected void stop() throws Exception {
		...
		for (ManagementEventListener lstr : this.management.getManagementEventListeners()) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error("Exception while invoking onAssociationStopped", ee);
			}
		}
	...
	protected void markAssociationUp() {
		...
		for (ManagementEventListener lstr : this.management.getManagementEventListeners()) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error("Exception while invoking onAssociationUp", ee);
			}
		}
	}
	...
	protected void markAssociationDown() {
		...
		for (ManagementEventListener lstr : this.management.getManagementEventListeners()) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error("Exception while invoking onAssociationDown", ee);
			}
		}
	...
	protected void read() {
		...
		try {
			...
			if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
				MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Rx : Ass=%s %s", this.name, payload));
			}
			...
			if (this.management.isSingleThread()) {
				...
				try {
					...
					MyLoggerFactory.getInstance().getAPILogger().error(String.format("Error while calling Listener for Association=%s.Payload=%s", this.name,
							payload), e);
				}
			} else {
...
//						+ this.workerThreadTable[payload.getStreamNumber()]);
				...
				try {
					...
					MyLoggerFactory.getInstance().getAPILogger().error(String.format("Rejected %s as Executors is shutdown", payload), e);
				} catch (NullPointerException e) {
					MyLoggerFactory.getInstance().getAPILogger().error(String.format("NullPointerException while submitting %s", payload), e);
				} catch (Exception e) {
					MyLoggerFactory.getInstance().getAPILogger().error(String.format("Exception while submitting %s", payload), e);
				}
			}
		} catch (IOException e) {
			this.ioErrors++;
	...
	private PayloadData doReadSctp() throws IOException {
		...
		if (messageInfo == null) {
			if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
				MyLoggerFactory.getInstance().getAPILogger().debug(String.format(" messageInfo is null for Association=%s", this.name));
			}
			return null;
		}
		...
		if (len == -1) {
			MyLoggerFactory.getInstance().getAPILogger().error(String.format("Rx -1 while trying to read from underlying socket for Association=%s ",
					this.name));
			this.close();
			this.scheduleConnect();
			return null;
	...
	private PayloadData doReadTcp() throws IOException {
		...
		if (len == -1) {
			MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Rx -1 while trying to read from underlying socket for Association=%s ",
					this.name));
			this.close();
			this.scheduleConnect();
			return null;
		...
		try {			
			...
			if (!txQueue.isEmpty()) {
				while (!txQueue.isEmpty()) {
					...
					if (MyLoggerFactory.getInstance().getAPILogger().isDebugEnabled()) {
						MyLoggerFactory.getInstance().getAPILogger().debug(String.format("Tx : Ass=%s %s", this.name, payloadData));
					}
	...
	protected void close() {
		if (this.getSocketChannel() != null) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error(String.format("Exception while closing the SctpScoket for Association=%s", this.name), e);
			}
		}
		...
		if (this.txQueue.size() > 0) {
			MyLoggerFactory.getInstance().getAPILogger().warn(String.format("Clearig txQueue for Association=%s. %d messages still pending will be cleared",
	...
	protected void initiateConnection() throws IOException {
		...
		if (this.getSocketChannel() != null) {
			try {
				...
				MyLoggerFactory.getInstance().getAPILogger().error(
						String.format(
		...
		try {
			...
			MyLoggerFactory.getInstance().getAPILogger().error("Error while initiating a connection", e);
			this.scheduleConnect();
			return;
		}
