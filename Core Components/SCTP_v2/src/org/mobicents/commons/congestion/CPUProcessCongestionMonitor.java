...
public class CPUProcessCongestionMonitor implements CongestionMonitor {
	...
	public void setBackToNormalCPUThreshold(double backToNormalCPUThreshold) {
		...
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info("Back To Normal CPU threshold set to " + backToNormalCPUThreshold + "%");
		}
	}
	...
	public void setCPUThreshold(double cpuThreshold) {
		...
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info("CPU threshold set to " + this.cpuThreshold + "%");
		}
	}
	...
	public void monitor() {
		...
		if(MyLoggerFactory.getInstance().getAPILogger().isTraceEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().trace("CPU Process Load Average = " + percentageOfCPUUsed);
		}
		if (this.cpuTooHigh) {
			if (this.percentageOfCPUUsed < this.backToNormalCPUThreshold) {
				MyLoggerFactory.getInstance().getAPILogger().warn("CPU used: " + percentageOfCPUUsed + "% < to the back to normal CPU threshold : " + this.backToNormalCPUThreshold);
			...
			if (this.percentageOfCPUUsed > cpuThreshold) {
				MyLoggerFactory.getInstance().getAPILogger().warn("CPU used: " + percentageOfCPUUsed + "% > to the CPU threshold : " + this.cpuThreshold);
