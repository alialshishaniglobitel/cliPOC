...
public class MemoryCongestionMonitor implements CongestionMonitor {
	...
	public void setBackToNormalMemoryThreshold(int backToNormalMemoryThreshold) {
		...
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info("Back To Normal Memory threshold set to " + backToNormalMemoryThreshold + "%");
		}
	}
	...
	public void setMemoryThreshold(int memoryThreshold) {
		...
		if (MyLoggerFactory.getInstance().getAPILogger().isInfoEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().info("Memory threshold set to " + this.memoryThreshold + "%");
		}
	}
	...
	public void monitor() {
		...
		if(MyLoggerFactory.getInstance().getAPILogger().isTraceEnabled()) {
			MyLoggerFactory.getInstance().getAPILogger().trace("Percentage of Memory Used = " + percentageOfMemoryUsed);
		}
		if (this.memoryTooHigh) {
			if (this.percentageOfMemoryUsed < this.backToNormalMemoryThreshold) {
				MyLoggerFactory.getInstance().getAPILogger().warn("Memory used: " + percentageOfMemoryUsed + "% < to the back to normal memory threshold : " + this.backToNormalMemoryThreshold);
			...
			if (this.percentageOfMemoryUsed > memoryThreshold) {
				MyLoggerFactory.getInstance().getAPILogger().warn("Memory used: " + percentageOfMemoryUsed + "% > to the memory threshold : " + this.memoryThreshold);
